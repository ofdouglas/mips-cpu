...
entity program_counter is
  port (clk              : in std_logic;
        ...
        pc_ibus_addr     : out unsigned(31 downto 0);
...
begin
  ...
  ibus_addr_proc:
  process (reset, exception_flush, exception_vector, pc_enable, pc_next, pc)
  begin
    if reset = '1' then
      pc_ibus_addr <= EXC_RESET_VADDR;
    elsif exception_flush = '1' then
      pc_ibus_addr <= exception_vector;
    elsif pc_enable = '1' then
      pc_ibus_addr <= pc_next;
    else
      pc_ibus_addr <= pc;
    end if;

    pc_ibus_addr(1 downto 0) <= "00";
  end process;
