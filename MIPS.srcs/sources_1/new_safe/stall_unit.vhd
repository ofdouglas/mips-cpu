...
entity stall_unit is
  port (EX_MEM_do_load       : in std_logic;
        ...
        ID_pc_next_mux       : in pc_next_mux_type;
...
-- NOTE: we actually should not stall when 'needed' register is r0!
  ...
  process (EX_MEM_do_load, EX_MEM_reg_dest_num, EX_do_rs_read, EX_do_rt_read,
           ID_EX_rs_num, ID_EX_rt_num, ID_rs_num, ID_rt_num)
  begin
    load_use_stall <= '0';

    if EX_MEM_do_load = '1' then
      -- ID Stage
  ...
  process (ID_EX_do_reg_write, ID_EX_reg_dest_num, ID_pc_next_mux,
           ID_rs_reg_num, ID_do_rs_read, ID_rt_reg_num, ID_do_rt_read)
  begin
    jump_branch_stall <= '0';
    
    if ID_EX_do_reg_write = '1' and ID_pc_next_mux /= PC_DO_PLUS_4 then
